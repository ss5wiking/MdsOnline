...
import './index.scss';
import {Toast,List,Picker,Icon,ActivityIndicator} from 'antd-mobile';
...
import SelectorModalNew from "components/SelectorModalNew";
import WhiteNo from "@/components/WhiteNo";

import down from "images/down.svg";
import yellowstar from "images/yellowstar.svg";
export default class ExpertConsultation extends Component {
	constructor(props){
		super(props)
		this.state = {
			SelectorModalvisible:false,
			SelectorModalvisibleNew:false,
			list:null,
			visible1:false,
			visible3:false,
			visibleValue1:'全部职称',
	...
	componentDidMount(){
			...
			_fetch(api.doctorsPath,{filter:"pro"})
	    .then(result=>{
	...
	SelectorModalStatusNew = ()=>{
		this.setState({
			SelectorModalvisibleNew:true
		})
	}

	citySelect =(id,name)=>{
		this.setState({SelectorModalvisible:false,city:id,visibleValue2:name},()=>{
			this.getData()
		})
	}

	depSelect =(id,name)=>{
		this.setState({SelectorModalvisibleNew:false,department:id,visibleValue4:name},()=>{
			this.getData()
		})

	}
	getData = ()=>{
		const {title,sort,city,department} =this.state
		_fetch(api.doctorsPath,{ title,sort,city,department, filter:"pro" })
		.then(result=>{
			if(result.status=="success"){
				this.setState({
					list:result.data
		...
		return (
			<div className="ExpertConsultation">
				<ul className="select clearfix">
					<li className="pull-left clearfix" onClick={()=>this.SelectorModalStatus()}>
						<div className="select_name pull-left">{visibleValue2}</div><img className="pull-left" src={down} alt=""/>
					</li>
					<li className="pull-left clearfix" onClick={()=>this.SelectorModalStatusNew()} >
				...
				<SelectorModal
					SelectorModalvisible = {this.state.SelectorModalvisible}
					onCancel={()=>this.setState({SelectorModalvisible:false})}
					onSub={(id,name)=>this.citySelect(id,name)}
					className={!this.state.SelectorModalvisible && 'up'}
				/>

				<SelectorModalNew
					SelectorModalvisibleNew = {this.state.SelectorModalvisibleNew}
					onCancelNew={()=>this.setState({SelectorModalvisibleNew:false})}
					onUp={(id,name)=>this.depSelect(id,name)}
					className={!this.state.SelectorModalvisibleNew && 'upNew'}
				/>
