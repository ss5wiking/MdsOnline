...
export default class AboutDoctor extends Component {
	...
	componentDidMount(){
			console.log(this.props , 696969)
			_fetch(api.doctorsPath,{department:this.props.location.state.department})
	    .then(result=>{
	...
	depSelect =(id,name)=>{
		this.setState({SelectorModalDepartment:false,department:id ,visibleValue4:name},()=>{
			this.getData()
		})
	...
	getData = ()=>{
		...
		_fetch(api.doctorsPath,{ title,sort,city,department })
		.then(result=>{
			if(result.status=="success"){
				this.setState({
					list:result.data
		...
		return (
			...
			<div className="ExpertConsultation">
				<ul className="select clearfix">
					...
					<li className="pull-left clearfix" onClick={()=>this.SelectorModalDepartment()} >
				...
				<SelectorModalDepartment
					SelectorModalDepartment = {this.state.SelectorModalDepartment}
					onCancelNew={()=>this.setState({SelectorModalDepartment:false})}
					onUp={(id,name)=>this.depSelect(id,name)}
					className={!this.state.SelectorModalDepartment && 'upNew'}
				/>
