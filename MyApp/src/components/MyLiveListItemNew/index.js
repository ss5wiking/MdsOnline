...
export default class MyLiveListItemNew extends Component {
  // 默认数据
  constructor(props) {
    super(props)
  }
  ...
  render() {
    return (
      <div className="MyLiveListItemNew">
        {
                  ...
                  <Link to={{pathname: `/lives/${item.id}`, state: item}}>
                    <div className="my_livelistitem_content clearfix">
                      <div className="img_wrap pull-left">
                        ...
                        <p className={item.live_status == "living" ? "videotape_status_liveing":"videotape_status_livewill"}>{item.live_status == "living" ? "直播中":"未开始"}</p>
                      </div>
