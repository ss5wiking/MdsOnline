...
import iScroll from "iscroll/build/iscroll-probe.js";
  ...
  export default class SelectorModalDepartment extends Component {
  // 默认数据
  constructor(props) {
    super(props)
    this.state = {
      Department: [{id:"",name:"全部科室",active:true}],
      Department_right: [{id: "", name: "全部科室"}],
    }
  }
  ...
  getDepartmentRight(DepartmentId ,index) {
    let right=[]
    if( index == 0 ){
      right =[{id: "", name: "全部科室"}]
    }else{
  ...
  render() {
    return (
      <div className={`${this.props.className} SelectorModalDepartment clearfix`}>
				<div className="title">
            ...
            <ul className="Selector_main">
              {
                this.state.Department.length>0 && this.state.Department.map( (item, index)=>(
                  <li key={index}  className={item.active==true && 'active'} onClick = {()=>this.getDepartmentRight(item.id,index)} >{item.name}</li>
                ))
              }
            </ul>
            </ReactIScroll>
        ...
        <div className="Selector_else_wrap pull-left">
            ...
            <div className="Selector_else">
            {
              this.state.Department_right.length>0 && this.state.Department_right.map( (item, index)=>(
                <Link to={{pathname:'/AboutDoctor' ,state: {chek:item.name , department:item.id}}} key={index}  className={item.action==true && 'action'}  onClick = {()=>this.getAction(item.id,item.name)}>{item.name}</Link>
              ))
            }
            </div>
            </ReactIScroll>
